...
def train_net(state,last_model,change=None):
...
def exec(state,path,num,last_model,change):
    ...
    args.dataset_dir = '/media/nas/DISK1/Apollo_Sim_3D_Lane_Release'
    args.data_dir = ops.join('/media/nas/DISK1/data_splits', args.dataset_name)
    args.save_path = ops.join(path+'/'+str(num),
                              args.dataset_name)
    ...
    # run the training
    db_entry, model = train_net(state, last_model, change)
    return db_entry, model
