...
def deploy(args, loader, dataset, model, vs_saver, test_gt_file, lane_pred_file, vis=False, vis_feat=False, epoch=0):
    ...
    with torch.no_grad():
                ...
                try:
                    if vis_feat:
                        output_net, pred_hcam, pred_pitch,\
                            ...
                            x1_proj, x2_proj, x3_proj, x4_proj, top_2, top_3, top_4,time_ = model(input)
                    else:
                        output_net, pred_hcam, pred_pitch ,time_= model(input)
                        times.append(time_)
        ...
        return eval_stats,avg_time


def test_3DLane(state,path,num):
    ...
    # manual settings
    args.dataset_dir = '/media/nas/DISK1/Apollo_Sim_3D_Lane_Release'  # raw data dir
    ...
    model_path=path+'/'+str(num)
    # generate relative paths
    args.data_dir = ops.join('/media/nas/DISK1/data_splits', args.dataset_name)
    args.save_path = os.path.join(ops.join(model_path, args.dataset_name), args.mod)
    ...
    model = LaneNAS.Net(args, state=state, debug=vis_feat)
    ...
    # deploy the model
    eval_stats,avg_time = deploy(args, test_loader, test_dataset, model, vs_saver, test_gt_file, lane_pred_file, vis, vis_feat)
    print('eval_state:', eval_stats)
    return eval_stats, avg_time
